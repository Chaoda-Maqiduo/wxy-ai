...
from app.schemas.thesis import OutlinePayload
...
def _parse_and_validate_outline(raw: str) -> dict:
...
def _build_outline_instructions(
    ...
    return {
        ...
        "three_level_instruction": (
            "每个二级章节下需要拆分出 2-3 个三级小节"
            if three_level
            else "保持常规二级章节结构即可"
        ),
        "aboutmsg_instruction": (
            f"写作方向补充说明：{aboutmsg.strip()}"
            if aboutmsg and aboutmsg.strip()
            else "无额外写作方向补充说明"
        ),
    }
...
async def generate_outline(
    ...
    return _parse_and_validate_outline(result)
