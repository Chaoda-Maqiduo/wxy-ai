...
async def _query_one(client: httpx.AsyncClient, title: str) -> dict | None:
...
def _extract_crossref_fields(cr_item: dict) -> dict:
...
async def enrich_with_crossref(items: list[dict]) -> list[dict]:
    """
    ...
    enriched_count = 0
    for item, cr_item in zip(items, results):
