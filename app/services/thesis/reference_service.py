...
def _format_one_reference(item: dict, index: int, is_zh: bool) -> str:
    ...
    final_year = cr_year or _extract_year(
        summary, item.get("snippet", ""), item.get("publication_date", ""),
    )
    if not final_year:
        return ""
...
async def generate_references(
    ...
    if include_english:
        zh_filtered, en_filtered = await asyncio.gather(
            _filter_results(llm, title, zh_results, "中文",
                            keep_count=target_zh + buffer,
                            fallback_num=max(10, wxnum)),
            _filter_results(llm, title, en_results, "英文",
                            keep_count=target_en + buffer,
                            fallback_num=max(8, min(wxnum, 15))),
        )
    else:
        zh_filtered = await _filter_results(llm, title, zh_results, "中文",
                                            keep_count=target_zh + buffer,
                                            fallback_num=max(10, wxnum))
        en_filtered = []
    ...
    try:
        from app.services.thesis.crossref_client import enrich_with_crossref
    ...
    def _append_formatted(items: list[dict], limit: int, *, is_zh: bool) -> None:
        nonlocal idx
        for item in items:
    ...
    if len(lines) < target_total:
        # 回补的也需要 CrossRef 补全
        zh_remaining = [
            item for item in zh_results
            if _title_key(item) not in used_title_keys
        ]
        en_remaining = [
